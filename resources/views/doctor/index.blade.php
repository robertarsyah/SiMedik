                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-pink-50 text-pink-700 uppercase text-xs">
                            ...
                            <th class="px-28 py-4 font-bold border-b text-center">
                                Nama Pasien
                            </th>

                            <th class="px-6 py-4 font-bold border-b text-center">
                                Aksi
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($queues as $q)
                            <tr class="border-b bg-gray-50 transition">
                                <td class="px-6 py-4 text-center text-2xl text-pink-600">
                                    {{ $q->queue_number }}
                                </td>

                                <td class="px-6 py-4 text-center font-bold text-pink-600 uppercase">
                                    {{ $q->patient->name }}
                                </td>

                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('doctor.examine', $q->id) }}"
                                        class="bg-pink-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-pink-700 transition shadow-sm active:scale-95 inline-block text-xs uppercase tracking-tight">
                                        MULAI PERIKSA
                                    </a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-10 text-center text-gray-400 bg-gray-50 italic">
